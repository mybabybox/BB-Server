...
public class FollowEventListener {
    ...
    public void recordFollowEventInDB(FollowEvent map){
		...
		if (localUser.onFollow(user)) {
			...
			CalcServer.getInstanceForDI().addToFollowQueue(localUser.id, user.id, score.doubleValue());
			CalcServer.getInstanceForDI().isFollowed(user.id, localUser.id);
    ...
    public void recordUnFollowEventInDB(UnFollowEvent map){
		...
		if (localUser.onUnFollow(user)) {
			CalcServer.getInstanceForDI().removeFromFollowQueue(localUser.id, user.id);
		}
    }
}
